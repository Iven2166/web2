...
利用"曝光未点击"作为负样本，在排序阶段是好的，但在召回阶段，就违背了"离线训练数据分布应当和线上服务的数据分布一致"的原理。两者目标不同：

- 排序目标：在用户喜欢里，进行优中选优，进行排序
- 召回目标：将用户喜欢和用户绝对不喜欢进行分隔，而曝光未点击属于困难样本（hard negative）
...
排序要求有预测的准确率，所以使用 binary cross-entropy loss ，希望将每个样本的CTR/CVR预测得更好。

召回的优化目标，损失函数主要考虑的是"相对准确性"

（1）NCE

使用的是 NCE（noise contrastive estimation） loss ，在一个batch里交互过的物料去做 softmax loss，最大化当前的选中目标的概率（类似于NLP或者CLIP的对比学习损失函数）

NCE是简化分母的思路，将召回里原始的"超大规模多分类问题"简化为一系列"区分样本是否来自于噪声"的二分类问题。

负采样概率：由于随机采样，由于少数热门物料占据大量的曝光点击，可能带偏模型对所有用户只出热门物料，从而丧失推荐的个性化。修正的目的是为了打压召回结果里的热门物料，所以有负采样概率（是物料热度的递增函数）

（2）Sampled Softmax Loss

（3）Pairwise Loss

用户和正样本（点击过的物料），要比负样本（随机采样得到的物料）的匹配程度，高出一定的阈值

## 5.3 word2vec

item2vec

将用户的行为序列，当作一个句子，序列里的每一个item id 当作一个单词，

- Airbnb的 I2I 召回、U2I 召回
- 阿里的 ECES (Enhanced Graph Embedding with Side Information) 召回
...
任何一个推荐系统，都难逃"2~8"定律的影响，即20%的热门ite占据了80%的曝光与点击。因为正例中的物料都是用户点击过的物料，可以说正例数据被少数热门物料所"垄断"。导致的后果就是，训练时，模型会迫使每个用户向量尽可能接近少数热门物料的向量；预测时，每个用户从FASS检索出来的邻居都是那少数几个热门物料，使推荐结果丧失了个性化与多样性。由此可见，"打压热门物料"的必要性。视物料出现在正例还是在负例中，要采取截然不同的打压策略。

- 热门物料当正例要降采样：物料热度越高，让它越不容易成为正例
- 热门物料当负例要过采样：1、由于训练时热门物料，统治了正样本，所以反过来在负例进行过采样 2、让热门物料成为 hard negative，而不全都是随机采样的 easy negative
